...
export function buildArtifactInstruction(
  ...
  return [
    ...
    "The smoke runtime can execute between one and eight declared click actions in order. Each click must synchronously advance the expected interaction without timers or layout measurements.",
    'For a calculator keypad, declare the natural 7, +, 1, = click sequence and expect text "8" in the result. The digit and operator handlers must build the same expression that Equals evaluates: immediately before Equals, that expression must be exactly 7+1. For input-based calculators, explicit HTML values and one Calculate click are also valid.',
    'manifest.json must be a JSON-encoded string shaped exactly like {"entry":"index.html","ui":{"preset":"pico-2"},"smoke":{"actions":[{"action":"click","selector":"#increment"}],"expect":{"selector":"#count","text":"1"}}}. The preset must be one of the listed preset IDs; smoke must contain one to eight bounded click actions and every selector must be an ID present in index.html.',
    input.baseArtifact
